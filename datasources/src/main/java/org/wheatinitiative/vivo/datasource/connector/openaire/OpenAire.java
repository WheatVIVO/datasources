...
public class OpenAire extends ConnectorDataSource implements DataSource {
    ...
    /*
     * We still have to filter the results, because we can't use a query term at the protocol level.
     */
    ...
    private class OaiModelIterator implements IteratorWithSize<Model> {
        ...
        public OaiModelIterator(String repositoryURL, List<String> metadataPrefixes) 
        ...
        public Model next() {
            if(cachedResult != null) {
                ...
                if(projectsDone && pubsDone) {
                	...
                	// At the moment let's concentrate on projects and publications
        ...
        private Model fetchNextProject(boolean cacheResult) {
            ...
            if(resumptionToken != null) {
                ...
                uriB.addParameter("metadataPrefix", metadataPrefixes.get(0));//Only the 1st prefix for this.
            }
            try {
        ...
        private Model fetchNextPublication( boolean cacheResult, String givenPrefix ) {
            ...
            if(resumptionToken != null) {
            	...
            	uriB.addParameter("set", "openaire");	// "openaire" -> set for all the pubs
	...
	protected Model filter(Model model) {
		// TODO - Filter the retrieved results so that we work only with the wheat-related data.
		return model;
	}
    ...
    protected Model renameByIdentifier(Model model) {
    	model = renameByIdentifier(model, model.getProperty(
    			OPEN_AIRE_TBOX_NS + "identifier"), "id");
        model = renameByIdentifier(model, model.getProperty(
        		OPEN_AIRE_TBOX_NS + "inraIdentifier"), "in");
        model = renameByIdentifier(model, model.getProperty(
        		OPEN_AIRE_TBOX_NS + "idCollection"), "ic");
        return model;
    }
    
    private Model renameByIdentifier(Model model, Property identifier, 
        ...
        for(Resource res : idMap.keySet()) {
            ResourceUtils.renameResource(
                    res, OPEN_AIRE_ABOX_NS + localNamePrefix + idMap.get(res));
        }
        return model;
    }
